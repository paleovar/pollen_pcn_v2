...
`main_fit_emulators.R` | fitting of emulators (general adversarial models) for TraCE and HadCM3 simulations
`main_pca.R` | principal component analysis of ACER AP pollen proxies and TraCE surrogates
...
`create_fig[...].R` | [R](https://www.r-project.org/) scripts to create the individual figures included in the main paper (figures starting with a number) and the in the supplementary material (figures starting with `S`). Figures which share greater parts of code and/or objects are combined within a single script.
...
`data_in` | directory for raw input data like th output climate simulations
...
We use corrected d18O data on the AICC2012 chronology which was compiled by Peter Köhler (peter.koehler@awi.de) and obtained by personal communication. 
...
- `zoo`
- `netcdf4`
- `nest` (can be obtained from `https://github.com/krehfeld/nest`)
- `PaleoSpec` (can be obtained `https://github.com/EarthSystemDiagnostics/paleospec`)
- `vegan`
- `palinsol`
- `mvnfast`
- `mgcv`
